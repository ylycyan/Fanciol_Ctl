...
static uint8_t TimeV2_DaysInMonth(uint16_t year, uint8_t month)
{
    ...
    if(month == 2U && TimeV2_IsLeapYear(year)) days++;
    return days;
}
...
uint8_t TimeV2_FromUnix(uint32_t timestamp, time_v2_fields_t *fields)
{
    ...
    for(;;) {
        uint16_t daysInYear = TimeV2_IsLeapYear(year) ? 366U : 365U;
    ...
    for(;;) {
        uint8_t daysInMonth = TimeV2_DaysInMonth(year, month);
...
uint8_t TimeV2_ToUnix(const time_v2_fields_t *fields, uint32_t *timestamp)
{
    ...
    for(month = 1U; month < fields->month; month++) {
        days += TimeV2_DaysInMonth(fields->year, month);
    }
