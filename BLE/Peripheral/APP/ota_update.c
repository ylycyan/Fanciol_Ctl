...
void Ota_Init(void)
{
    ...
    if(!found) {
        ...
        metadata_slot = OTA_METADATA_B;
        (void)metadata_save();
    } else if(metadata.state == OTA_STATE_DOWNLOADING) {
        if(metadata.url_length == 0U) {
            /* A phone-side transfer cannot resume after reset. */
...
uint8_t Ota_Write(uint32_t offset, const uint8_t *data, uint16_t length)
{
    ...
    if(FLASH_ROM_WRITE(OTA_STAGING_ADDRESS + offset, (uint8_t *)data, length) != 0U ||
       FLASH_ROM_VERIFY(OTA_STAGING_ADDRESS + offset, (uint8_t *)data, length) != 0U)
        return DEVICE_STATUS_IO_ERROR;
