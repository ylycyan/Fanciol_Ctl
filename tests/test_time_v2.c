                             ...
                             uint8_t second)
{
    ...
    assert(TimeV2_FromUnix(timestamp, &fields));
    ...
    assert(TimeV2_ToUnix(&fields, &roundTrip));
    assert(roundTrip == timestamp);
}

int main(void)
{
    time_v2_fields_t invalid = {2023U, 2U, 29U, 0U, 0U, 0U};
    time_v2_fields_t leap = {2024U, 2U, 29U, 0U, 0U, 0U};
    time_v2_fields_t tooLate = {2038U, 1U, 20U, 0U, 0U, 0U};
    uint32_t timestamp;
    ...
    assert(!TimeV2_ToUnix(&invalid, &timestamp));
    assert(TimeV2_ToUnix(&leap, &timestamp));
    assert(timestamp == 1709164800UL);
    assert(!TimeV2_ToUnix(&tooLate, &timestamp));
    assert(!TimeV2_FromUnix(2147483001UL, &invalid));

    puts("time_v2 tests passed");
    return 0;
}
