...
int main(void)
{
    ...
    assert(V2_BleEncode(&b,out,sizeof(out),&len)==V2_STATUS_OK);
    {v2_ble_reassembler_t reassembler;uint8_t count=(uint8_t)((len+14u)/15u),index;V2_ReassemblerReset(&reassembler);
        for(index=0;index<count;index++){uint16_t offset=(uint16_t)index*15u,chunk=(uint16_t)(len-offset);if(chunk>15u)chunk=15u;
            ...
            assert(V2_ReassemblerPush(&reassembler,fragment,(uint16_t)(chunk+5u),assembled,sizeof(assembled),&assembled_len)==(index==count-1?V2_STATUS_OK:V2_STATUS_BUSY));}
        assert(assembled_len==len&&memcmp(assembled,out,len)==0);
        fragment[0]=0;fragment[1]=0;fragment[2]=1;assert(V2_ReassemblerPush(&reassembler,fragment,5,assembled,sizeof(assembled),&assembled_len)==V2_STATUS_INVALID_ARG);}
    puts("CH583 BLE protocol V2 golden vectors: PASS"); return 0;
}
